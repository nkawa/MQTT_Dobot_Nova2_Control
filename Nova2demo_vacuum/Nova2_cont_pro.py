        ...
        while self.loop:
            ...
            if(dt > 20):
                ...
                if(default_pos_flag):
                    ...
                    for i in range(6): 
                        msg_angle[i] = str(in_angle[i])  
                    ret = self.client_move.sendRecvMsg("JointMovJ("+msg_angle[0]+","+msg_angle[1]+","+msg_angle[2]+","+msg_angle[3]+","+msg_angle[4]+","+msg_angle[5]+")")
                if(is_JointMovJ):
                    rounded_goal_dif = [round(num,3) for num in goal_dif]
                    is_all_small = all(abs(x) <= 0.25 for x in rounded_goal_dif)
                    print(rounded_goal_dif)
                    if is_all_small:
                        is_JointMovJ = False
                        print("------------ finish JointMovJ")
                else:
